...
const PRICE = 6250; // VPT per Ether
const SUPPLY_LIMIT = 312500000; // VPT Cap in ether (not wei) units
...
contract("PresaleToken", () => {
    ...
    token.buyTokens(investor1, {value: web3.toWei(1, 'ether'), from: investor1})
      .then(() => {
        token.balanceOf.call(investor1).then(res =>
          assert.equal(PRICE, web3.fromWei(res.toFixed(), 'ether'),
            "1 Ether should buy "+PRICE+" VPT"))
  ...
  it("should fail buyTokens beyond token limit in Phase.Running", () =>
    token.buyTokens(investor1, {value: web3.toWei(SUPPLY_LIMIT+1, 'ether'), from: investor1}) // requires super rich investor1
      .then(assert.fail).catch(evmThrow))
  ...
  it("can call buyTokens in Phase.Running again", () =>
    token.buyTokens(investor2, {value: web3.toWei(1, 'ether'), from: investor2})
      .then(() => {
        token.balanceOf.call(investor2).then(res =>
          assert.equal(2*PRICE, web3.fromWei(res.toFixed(), 'ether'),
            "1 Ether should buy "+PRICE+" DPT"))
        const balance = web3.eth.getBalance(token.address)
        return assert.equal(1, web3.fromWei(balance.toFixed(), 'ether'), "contract balance is 1 ether")
      }))
